...
class TestOutputParser(unittest.TestCase):
    ...
    def test_decomplexify(self):
        """
        Checks the decomplexify parameter properly forces the correct detailed mode switches to 1.
        Makes sure when all zeroes are passed in the detailed mode switches are still on.
        And makes sure when all ones are passed in the detailed mode switches are correctly on.
        """
        all_actions = list(self.input_specs["varId"])

        output_parser = OutputParser(all_actions, decomplexify=True)

        # Pass all zeroes through output parser, the switches that enable detailed mode should still be on.
        all_zeroes = torch.zeros((1, len(all_actions)))
        zeroes_parsed = output_parser.parse_output(all_zeroes)
        actions_dict_zeroes = dict(zip(all_actions, zeroes_parsed[0].tolist()))

        # Get the switches that should always be on
        condition = (
            (self.input_specs["kind"] == "switch") &
            (self.input_specs["varId"] != "_electric_standard_active") &
            (self.input_specs["slidersActiveWhenOn"].apply(lambda x: isinstance(x, list) and len(x) > 0))
        )
        detailed_actions = self.input_specs.loc[condition, "varId"].tolist()

        # Check that all the detailed switches are on despite being passed 0s.
        for action in detailed_actions:
            true_on_value = self.input_specs[self.input_specs["varId"] == action]["onValue"].values[0]
            self.assertEqual(actions_dict_zeroes[action], true_on_value)

        # Just for fun pass all ones through the output parser too
        all_ones = torch.ones((1, len(all_actions)))
        ones_parsed = output_parser.parse_output(all_ones)
        actions_dict_ones = dict(zip(all_actions, ones_parsed[0].tolist()))

        # Check that all the detailed switches are correctly on.
        for action in detailed_actions:
            true_on_value = self.input_specs[self.input_specs["varId"] == action]["onValue"].values[0]
            self.assertEqual(actions_dict_ones[action], true_on_value)
