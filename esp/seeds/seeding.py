...
def train_seed(model_params: dict,
    ...
    prescriptor = EnROADSPrescriptor(model_params, DEVICE)
    model = prescriptor.model
    model.to(DEVICE)
    model.train()
...
def create_seeds(save_dir: Path,
                 model_params: dict,
                 context_ds: Dataset,
                 actions: list[str],
    ...
    for i, label in enumerate(labels):
        prescriptor = train_seed(model_params, context_ds, label, epochs=epochs)
        prescriptor.save(save_dir / f"1_{i}")
